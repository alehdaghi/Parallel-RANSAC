...
static TYPENUM sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrarg*sqrarg)
static TYPENUM maxarg1,maxarg2;
...
Matrix::Matrix (const int32_t m_,const int32_t n_,const TYPENUM* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
Matrix::Matrix (const Matrix &M) {
  ...
  for (int32_t i=0; i<M.m; i++)
    memcpy(val[i],M.val[i],M.n*sizeof(TYPENUM));
}
...
void Matrix::getData(TYPENUM* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(TYPENUM s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(TYPENUM s,int32_t i1,int32_t i2) {
...
Matrix Matrix::rotMatX (const TYPENUM &angle) {
  TYPENUM s = sin(angle);
  TYPENUM c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Matrix Matrix::rotMatY (const TYPENUM &angle) {
  TYPENUM s = sin(angle);
  TYPENUM c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const TYPENUM &angle) {
  TYPENUM s = sin(angle);
  TYPENUM c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::operator* (const TYPENUM &s) {
  Matrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
bool Matrix::solve (const Matrix &M, TYPENUM eps) {
...
bool Matrix::lu(int32_t *idx, TYPENUM &d, TYPENUM eps) {
  ...
  int32_t i,imax,j,k;
  TYPENUM   big,dum,sum,temp;
  TYPENUM* vv = (TYPENUM*)malloc(n*sizeof(TYPENUM)); // vv stores the implicit scaling of each row.
  d = 1.0;
...
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  TYPENUM* w   = (TYPENUM*)malloc(n*sizeof(TYPENUM));
  TYPENUM* rv1 = (TYPENUM*)malloc(n*sizeof(TYPENUM));

  int32_t flag,i,its,j,jj,k,l,nm;
  TYPENUM   anorm,c,f,g,h,s,scale,x,y,z;
  ...
  for (k=n-1;k>=0;k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
    for (its=0;its<30;its++) { // and over allowed iterations.
      ...
      for (l=k;l>=0;l--) { // Test for splitting.
        nm = l-1;
        if ((TYPENUM)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
        if ((TYPENUM)(fabs( w[nm])+anorm) == anorm) { break; }
      }
      if (flag) {
        ...
        for (i=l;i<=k;i++) {
          f = s*rv1[i];
          rv1[i] = c*rv1[i];
          if ((TYPENUM)(fabs(f)+anorm) == anorm) break;
          g = w[i];
          h = pythag(f,g);
          w[i] = h;
          h = 1.0/h;
  ...
  int32_t s2,inc=1;
  TYPENUM   sw;
  TYPENUM* su = (TYPENUM*)malloc(m*sizeof(TYPENUM));
  TYPENUM* sv = (TYPENUM*)malloc(n*sizeof(TYPENUM));
...
void Matrix::allocateMemory (const int32_t m_,const int32_t n_) {
  ...
  val    = (TYPENUM**)malloc(m*sizeof(TYPENUM*));
  val[0] = (TYPENUM*)calloc(m*n,sizeof(TYPENUM));
...
TYPENUM Matrix::pythag(TYPENUM a,TYPENUM b) {
  TYPENUM absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
