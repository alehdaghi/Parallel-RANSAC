...
typedef double TYPENUM;      // double precision
//typedef float  TYPENUM;    // single precision
namespace Utility{
class Matrix {
...
public:
  ...
  Matrix (const int32_t m,const int32_t n,const TYPENUM* val_); // init mxn matrix with values from array 'val'
  ...
  void getData(TYPENUM* val_,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
  ...
  void setVal(TYPENUM s,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(TYPENUM s,int32_t i1=0,int32_t i2=-1);
  ...
  static Matrix rotMatX(const TYPENUM &angle);
  static Matrix rotMatY(const TYPENUM &angle);
  static Matrix rotMatZ(const TYPENUM &angle);
  ...
  Matrix  operator* (const TYPENUM &s);  // multiply with scalar
  Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
  Matrix  operator/ (const TYPENUM &s);  // divide by scalar
  Matrix  operator- ();                // negative matrix
  Matrix  operator~ ();                // transpose
  TYPENUM   l2norm ();                   // euclidean norm (vectors) / frobenius norm (matrices)
  TYPENUM   mean ();                     // mean of all elements in matrix
  ...
  TYPENUM  det ();                                             // returns determinant of matrix
  bool   solve (const Matrix &M,TYPENUM eps=1e-20);            // solve linear system M*x=B, replaces *this and M
  bool   lu(int32_t *idx, TYPENUM &d, TYPENUM eps=1e-20);        // replace *this by lower upper decomposition
...
private:
  ...
  void releaseMemory ();
  inline TYPENUM pythag(TYPENUM a,TYPENUM b);
